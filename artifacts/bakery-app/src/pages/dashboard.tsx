import { Loader2, DollarSign, ShoppingBag, CheckCircle, Clock, TrendingUp, Users, CalendarCheck, AlertCircle } from 'lucide-react';
...
function StatCard({ title, value, icon: Icon, color, delay }: {
  title: string; value: string | number; icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }>; color: string; delay: number;
}) {
  return (
    <motion.div
      ...
      style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(12px)', boxShadow: '0 2px 16px rgba(0,0,0,0.2)' }}
    >
...
export default function Dashboard() {
  ...
  const statCards = [
    ...
    { title: t.totalRevenue,     value: `₹${stats.total_revenue.toFixed(0)}`,            icon: DollarSign,    color: '#d4a844' },
    { title: t.thisMonth,        value: `₹${stats.this_month_revenue.toFixed(0)}`,       icon: TrendingUp,    color: '#4ade80' },
    { title: t.pendingPayments,  value: `₹${stats.pending_payments.toFixed(0)}`,         icon: DollarSign,    color: '#f87171' },
    ...
    <div className="px-4 py-5 space-y-5 max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-4 px-4 pb-4">
            <div className="h-44 flex-1">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={breakdown} cx="50%" cy="50%" innerRadius={48} outerRadius={72}
                    paddingAngle={4} dataKey="count" nameKey="category">
                    {breakdown.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={{
                    backgroundColor: 'rgba(20,12,4,0.95)', borderColor: 'rgba(212,168,68,0.2)',
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          ...
          {!actLoading && (
            <span className="text-[10px] text-muted-foreground px-1.5 py-0.5 rounded-md bg-white/5">
              Live
