...
function autoPayStatus(total: number, advance: number) {
  ...
  if (rem <= 0 && (advance > 0 || total === 0)) return { label: 'Paid', color: '#34d399', bg: 'rgba(52,211,153,0.12)' };
  if (advance > 0) return { label: 'Partial', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' };
  return { label: 'Unpaid', color: '#f87171', bg: 'rgba(248,113,113,0.12)' };
}
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        {/* Auto payment badge */}
        <div className="flex items-center justify-between rounded-xl p-3.5 border border-white/8"
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="submit" disabled={isPending || isSuccess}
            ...
            style={{ background: isSuccess ? '#34d399' : '#d4a844', color: '#0f0d0b' }}>
            {isPending ? (
